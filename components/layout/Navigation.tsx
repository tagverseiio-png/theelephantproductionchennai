        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          {content?.site?.title && (
            <div className="flex flex-col cursor-pointer group interactive" onClick={() => setActivePage('home')}>
              <span className={`font-serif text-2xl md:text-3xl tracking-widest font-semibold transition-colors duration-200 ${isScrolled || activePage !== 'home' ? 'text-[#2c2420]' : 'text-white'}`}>{content.site.title}</span>
              {content.site.subtitle && (
                <div className="flex items-center gap-2 transition-all duration-200">
                  ...
                  <span className={`text-[10px] tracking-[0.3em] uppercase text-center transition-colors ${isScrolled || activePage !== 'home' ? 'text-[#a67b5b]' : 'text-white/80'}`}>{content.site.subtitle}</span>
                </div>
              )}
            </div>
          )}
