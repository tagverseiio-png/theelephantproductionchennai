...
export const HomePage = ({ setPage }: HomePageProps) => {
  ...
  <div className="animate-in fade-in duration-500">
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-24 items-center">
          <div className="relative group">
            {content?.home?.philosophy?.imageUrl && (
              <Reveal>
                <div className="w-full h-[700px] relative z-10 shadow-2xl overflow-hidden">
                  <ParallaxImage 
                    src={normalizeMediaUrl(content.home.philosophy.imageUrl)} 
                    alt="Philosophy" 
          ...
          <div className="space-y-12">
            ...
            {content?.home?.philosophy?.ctaText && (
              <Reveal delay={300}>
                <button onClick={() => setPage('about')} className="group text-[#2c2420] text-sm uppercase tracking-widest font-bold flex items-center gap-4 interactive">
                  {content.home.philosophy.ctaText} <span className="group-hover:translate-x-2 transition-transform duration-200">→</span>
                </button>
              </Reveal>
            )}
          </div>
    ...
    <section className="py-32 bg-[#e8e0d9]/20">
      <div className="container mx-auto px-6">
         {content?.home?.latestJournals?.title && (
           <Reveal><h2 className="text-4xl md:text-6xl font-serif text-[#2c2420] text-center mb-20">{content.home.latestJournals.title}</h2></Reveal>
         )}
